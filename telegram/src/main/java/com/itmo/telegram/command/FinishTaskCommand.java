...
public class FinishTaskCommand extends AbstractCommand{
    ...
    public FinishTaskCommand(UserService userService, TaskService taskService) {
        super("/done", "Usage: (/done (task_id)");
    ...
    public void handleCommand(MessageContext ctx, SilentSender sender) {
        try{
            ...
            taskService.endTask(id, user);
            sender.execute(new SendMessage(ctx.chatId().toString(),"Task " + args[1] + " has finished successfully!"));
